...
def batch_data(audio_file, n_frames, overlap):
    ...
    # extend the time length to 3s 
    if audio_rep.shape[0] < n_frames:
        zeros = np.zeros((int(n_frames - audio_rep.shape[0]), config.N_MELS), dtype=np.float16)
        audio_rep = np.concatenate((audio_rep, zeros), axis = 0)          
...
def extractor(file_name, model='MTT_musicnn', input_length=3, input_overlap=False, extract_features=True):
    ...
    # batching data
    print('Computing spectrogram and tags for file {}.'.format(file_name))
