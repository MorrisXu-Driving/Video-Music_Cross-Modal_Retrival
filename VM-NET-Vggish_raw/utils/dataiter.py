...
def FeatLoader(csv_path, data_dir, batch_size):
    ...
    for i, batch in enumerate(batchs):
        ...
        for j, aid in batch.iterrows():
            ...
            a_f.append(tf.convert_to_tensor(np.mean(\
                                   np.load(audio_path), axis = (2, 3), keepdims = False)))
    ...
    x_feats = tf.expand_dims(tf.stack(a_f, axis = 0), axis = 1) # shape: (batch, channels = 1, time, feature_dims)
    y_feats = tf.stack(v_f, axis = 0) # shape: (batch, channels = 1, time, feature_dims)
